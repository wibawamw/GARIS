...
public class HealthMailReportPanel extends JXPanel implements ActionListener {
    ...
    public HealthMailReportPanel(ArchieveMainframe mainframe) {
    ...
    private JXPanel createRightComponent() {
        ...
        String text = "Penjelasan Singkat\n"
                + "Daftar Surat Keluar Keterangan Sehat merupakan daftar semua surat yang dikeluarkan dinas beserta statusnya "
                + "pada dinas tertentu\n"
                + "Untuk melihat data Surat Keluar Keterangan Sehat pilih periode berdasarkan tanggal, berdasarkan bulan,"
                ...
                + "Untuk melakukan cetak daftar Surat Keluar Keterangan Sehat, dari data yang telah difilter sebelumnya (panel dibawah) "
        ...
        task.setTitle("Cetak Daftar Surat Keluar Keterangan Sehat");
    ...
    private class LoadPrintPanel extends SwingWorker<JasperPrint, Void> {
        ...
        private JasperReport loadReportFile() throws InterruptedException, JRException {
            ...
            String filename = "OtherMailReport.jrxml";
        ...
        private DefaultTableModel constructModel(ArrayList<HealthMail> data) throws InterruptedException, SQLException {
            ...
            model.addColumn("ReceiverAddress");
            ...
            if (!data.isEmpty()) {
                for (int i = 0; i < data.size() && !isCancelled(); i++) {
                    ...
                    HealthMail mail = data.get(i);
                    model.addRow(new Object[]{Integer.valueOf(i + 1), mail.getMailDate(),
                                mail.getMailNumber(), mail.getSubject(), mail.getReceiver(),
                                mail.getReceiverAddress(), mail.getDescription()});
        ...
        private Map constructParameter() throws InterruptedException {
            ...
            param.put("reportname", "Keterangan Sehat");
        ...
        protected JasperPrint doInBackground() throws Exception {
            try {
                ...
                ArrayList<HealthMail> mail = new ArrayList<HealthMail>();

                if (startDate != null && endDate != null) {
                    mail = logic.getHealthMail(mainframe.getSession(), startDate, endDate);
                } else if (month != 0 || year != 0) {
                    mail = logic.getHealthMail(mainframe.getSession(), month, year);
                }


                if (!mail.isEmpty()) {
                    JasperReport jrReport = loadReportFile();
                    DefaultTableModel model = constructModel(mail);
        ...
        protected void done() {
            try {
                ...
                JXErrorPane.showDialog(HealthMailReportPanel.this, info);
            }
            setProgress(100);
