 ...
 */
public class OutboxReportPanel extends JXPanel implements ActionListener {
    ...
    private JXPanel createRightComponent() {
        ...
        String text = "Penjelasan Singkat\n"
                + "Daftar Surat Keluar merupakan daftar semua surat yang dikeluarkan dinas beserta statusnya "
                + "pada dinas tertentu\n"
                + "Untuk melihat data Surat Keluar pilih periode berdasarkan tanggal, berdasarkan bulan,"
                ...
                + "Untuk melakukan cetak daftar Surat Keluar, dari data yang telah difilter sebelumnya (panel dibawah) "
        ...
        task.setTitle("Cetak Daftar Surat Keluar");
    ...
    private class LoadPrintPanel extends SwingWorker<JasperPrint, Void> {
        ...
        private JasperReport loadReportFile() throws InterruptedException, JRException {
            ...
            String filename = "OutboxReport.jrxml";
        ...
        private DefaultTableModel constructModel(ArrayList<Outbox> data) throws InterruptedException, SQLException {
            ...
            if (!data.isEmpty()) {
                for (int i = 0; i < data.size() && !isCancelled(); i++) {
                    ...
                    Outbox outbox = data.get(i);

                    ArrayList<OutboxDisposition> dispositions = logic.getOutboxDisposition(mainframe.getSession(), outbox.getIndex());
        ...
        protected void done() {
            try {
                ...
                JXErrorPane.showDialog(OutboxReportPanel.this, info);
            }
            setProgress(100);
