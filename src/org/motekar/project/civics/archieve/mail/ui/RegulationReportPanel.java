 ...
 */
public class RegulationReportPanel extends JXPanel implements ActionListener {
    ...
    public RegulationReportPanel(ArchieveMainframe mainframe) {
    ...
    private JXPanel createCenterComponent() {
        ...
        JXTitledPanel titledPanel = new JXTitledPanel("Daftar Peraturan Daerah");
    ...
    private JXPanel createRightComponent() {
        ...
        String text = "Penjelasan Singkat\n"
                + "Daftar Peraturan Daerah merupakan daftar semua peraturan daerah yang ada di daerah tersebut\n"
                + "Untuk melihat data Peraturan Daerah pilih periode berdasarkan tanggal, berdasarkan bulan,"
                ...
                + "Untuk melakukan cetak daftar Peraturan Daerah, dari data yang telah difilter sebelumnya (panel dibawah) "
        ...
        task.setTitle("Cetak Daftar Peraturan Daerah");
    ...
    private class LoadPrintPanel extends SwingWorker<JasperPrint, Void> {
        ...
        private JasperReport loadReportFile() throws InterruptedException, JRException {
            ...
            String filename = "RegulationReport.jrxml";
        ...
        private DefaultTableModel constructModel(ArrayList<Regulation> data) throws InterruptedException {

            DefaultTableModel model = new DefaultTableModel();
            model.addColumn("No");
            model.addColumn("RegulationNumber");
            model.addColumn("RegulationDate");
            ...
            if (!data.isEmpty()) {
                for (int i = 0; i < data.size() && !isCancelled(); i++) {
                    ...
                    Regulation outbox = data.get(i);

                    model.addRow(new Object[]{Integer.valueOf(i + 1), outbox.getRegulationNumber(),
                                outbox.getRegulationDate(), outbox.getDescription()});
        ...
        protected JasperPrint doInBackground() throws Exception {
            try {
                ...
                ArrayList<Regulation> outbox = new ArrayList<Regulation>();

                if (startDate != null && endDate != null) {
                    outbox = logic.getRegulation(mainframe.getSession(), startDate, endDate);
                } else if (month != 0 || year != 0) {
                    outbox = logic.getRegulation(mainframe.getSession(), month, year);
                }


                if (!outbox.isEmpty()) {
                    JasperReport jrReport = loadReportFile();
                    DefaultTableModel model = constructModel(outbox);
        ...
        protected void done() {
            try {
                ...
                JXErrorPane.showDialog(RegulationReportPanel.this, info);
            }
            setProgress(100);
