 ...
 */
public class DecreeReportPanel extends JXPanel implements ActionListener {
    ...
    public DecreeReportPanel(ArchieveMainframe mainframe) {
    ...
    private JXPanel createCenterComponent() {
        ...
        JXTitledPanel titledPanel = new JXTitledPanel("Daftar Surat Keputusan");
    ...
    private JXPanel createRightComponent() {
        ...
        String text = "Penjelasan Singkat\n"
                + "Daftar Surat Keputusan merupakan daftar semua surat keputusan yang dikeluarkan dinas\n"
                + "Untuk melihat data Surat Keputusan pilih periode berdasarkan tanggal, berdasarkan bulan,"
                ...
                + "Untuk melakukan cetak daftar Surat Keputusan, dari data yang telah difilter sebelumnya (panel dibawah) "
        ...
        task.setTitle("Cetak Daftar Surat Keputusan");
    ...
    private class LoadPrintPanel extends SwingWorker<JasperPrint, Void> {
        ...
        private JasperReport loadReportFile() throws InterruptedException, JRException {
            ...
            String filename = "DecreeReport.jrxml";
        ...
        private DefaultTableModel constructModel(ArrayList<Decree> data) throws InterruptedException {

            DefaultTableModel model = new DefaultTableModel();
            model.addColumn("No");
            model.addColumn("SKNumber");
            model.addColumn("SKDate");
            ...
            if (!data.isEmpty()) {
                for (int i = 0; i < data.size() && !isCancelled(); i++) {
                    ...
                    Decree outbox = data.get(i);

                    model.addRow(new Object[]{Integer.valueOf(i + 1), outbox.getDecreeNumber(),
                                outbox.getDecreeDate(), outbox.getDescription()});
        ...
        protected JasperPrint doInBackground() throws Exception {
            try {
                ...
                ArrayList<Decree> outbox = new ArrayList<Decree>();

                if (startDate != null && endDate != null) {
                    outbox = logic.getDecree(mainframe.getSession(), startDate, endDate);
                } else if (month != 0 || year != 0) {
                    outbox = logic.getDecree(mainframe.getSession(), month, year);
                }
        ...
        protected void done() {
            try {
                ...
                JXErrorPane.showDialog(DecreeReportPanel.this, info);
            }
            setProgress(100);
