...
public class MasterSQL extends CommonSQL {
    ...
    void insertCurriculumVitae(Connection conn, CurriculumVitae cv) throws SQLException {

        StringBuilder query = new StringBuilder();
        query.append("insert into curriculumvitae(employeeindex,filename,filebyte)").
                append("values(?,?,?)");
    ...
    void deleteCurriculumVitae(Connection conn, Long index) throws SQLException {
        PreparedStatement pstm = conn.prepareStatement("delete from curriculumvitae where employeeindex = ?");
        pstm.setLong(1, index);
        pstm.executeUpdate();
        pstm.close();
    }

    ArrayList<CurriculumVitae> getCurriculumVitae(Connection conn, Long index) throws SQLException {
        ArrayList<CurriculumVitae> curriculumVitaes = new ArrayList<CurriculumVitae>();

        StringBuilder query = new StringBuilder();
        query.append("select * from curriculumvitae ").
                append("where employeeindex = ? ");
        ...
        while (rs.next()) {
            ...
            vitae.setCvIndex(rs.getLong("cvindex"));
            vitae.setEmployeeIndex(rs.getLong("employeeindex"));
            vitae.setFileName(rs.getString("filename"));
            vitae.setFileStream(rs.getBytes("filebyte"));

            curriculumVitaes.add(vitae);
        }
    ...
    void insertStandardPrice(Connection conn, StandardPrice price) throws SQLException {
        StringBuilder query = new StringBuilder();
        query.append("insert into standardprice(transactionname,transactiontype,transporttype, ").
                append("departure,destination,price,notes)").
                append("values(?,?,?,?,?,?,?)");

        PreparedStatement pstm = conn.prepareStatement(query.toString());
        pstm.setString(1, price.getTransactionName());
        pstm.setInt(2, price.getTransactionType());
        pstm.setInt(3, price.getTransportType());
        pstm.setString(4, price.getDeparture());
        pstm.setString(5, price.getDestination());
        pstm.setBigDecimal(6, price.getPrice());
        pstm.setString(7, price.getNotes());
        pstm.executeUpdate();
        pstm.close();
    }
    ...
    void updateStandardPrice(Connection conn, Long oldIndex, StandardPrice price) throws SQLException {
        StringBuilder query = new StringBuilder();
        query.append("update standardprice set transactionname = ?, transactiontype = ?, ").
