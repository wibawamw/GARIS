...
public class ExpeditionPanel extends JXPanel implements ActionListener, ListSelectionListener {
    ...
    private void generateCode() {
        try {
            ...
            String code = logic.generatedExpeditionCode(mainframe.getSession(), year);
    ...
    private void setFormValues() {
        if (selectedExpedition != null) {
            try {
                ...
                if (selectedExpedition.getAssignedEmployee().isGorvernor()) {
                    position.append(selectedExpedition.getAssignedEmployee().getPositionNotes());
                } else if (selectedExpedition.getAssignedEmployee().getStrukturalAsString().equals("")) {
                ...
                if (obj != null) {
                    if (obj instanceof Employee) {
                        ...
                        if (employee.isGorvernor()) {
                            fieldPosition.setText(employee.getPositionNotes());
                        } else if (employee.getStrukturalAsString().equals("")) {
    ...
    private class LoadPrintPanel extends SwingWorker<JasperPrint, JRPrintPage> {
        ...
        protected JasperPrint doInBackground() throws Exception {
            try {
                ...
                if (expedition.getLetter().getCommander().isGorvernor()) {
                    ExpeditionProgressCommJasper epcj = new ExpeditionProgressCommJasper(expedition, expedition.getLetter().getCommander(), properties);
                    pages.addAll(epcj.getPages());
                } else {
                    ExpeditionProgressJasper epj = new ExpeditionProgressJasper(expedition, expedition.getLetter().getCommander(), properties);
