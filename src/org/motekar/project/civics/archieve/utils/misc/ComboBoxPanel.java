...
public class ComboBoxPanel extends JPanel {
    ...
    private int row = 1;
    //
    private Integer categoryType = Integer.valueOf(0);
    ...
    public ComboBoxPanel(Integer categoryType, Connection conn, Long session, ItemsCategory parentCategory, boolean viewFullName) {
    ...
    private Component createMainPanel() {
        ...
        if (!comboArray.isEmpty()) {
            if (parentCategory != null) {
                comboArray.get(comboArray.size() - 1).setSelectedItem(parentCategory);
            }
        }
    ...
    private ArrayList<ItemsCategory> loadData(JXComboBox parentCombo) {
        ...
        if (parentCombo == null) {
            try {
                iCategorys = logic.getItemsCategory(session, categoryType, ItemsCategory.ROOT_PARENT_INDEX);
            ...
            if (category != null) {
                try {
                    iCategorys = logic.getItemsCategory(session, categoryType, category.getIndex());
                } catch (SQLException ex) {
                    Exceptions.printStackTrace(ex);
                }
            }
