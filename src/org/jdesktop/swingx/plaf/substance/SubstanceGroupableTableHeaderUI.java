...
public class SubstanceGroupableTableHeaderUI extends BasicGroupableTableHeaderUI {
    ...
    protected Map<ColumnGroup, Object> selectedGroups;
    /**
    ...
    private StateTransitionMultiTracker<ColumnGroup> stateTransitionMultiTrackerGroup;
    /**
    ...
    public SubstanceGroupableTableHeaderUI() {
        ...
        this.stateTransitionMultiTrackerGroup = new StateTransitionMultiTracker<ColumnGroup>();
        selectedIndices = new HashMap<Integer, Object>();
        selectedGroups = new HashMap<ColumnGroup, Object>();
    }
        ...
        this.substanceFadeSelectionListener = new ListSelectionListener() {
            ...
            public void valueChanged(ListSelectionEvent e) {
                ...
                if (!(ui instanceof SubstanceTableUI)) {
                    ...
                    stateTransitionMultiTrackerGroup.clear();
                    return;
                }
        ...
        this.substancePropertyChangeListener = new PropertyChangeListener() {
            ...
            public void propertyChange(PropertyChangeEvent evt) {
                if ("table".equals(evt.getPropertyName())) {
                    ...
                    TableColumnModel oldModel = (evt.getOldValue() instanceof JTable) ? ((JTable) evt.getOldValue()).getColumnModel()
                            : null;
                    TableColumnModel newModel = (evt.getNewValue() instanceof JTable) ? ((JTable) evt.getNewValue()).getColumnModel()
                            : null;
...
//                cellRect.y = 0;
//                
//                List<ColumnGroup> groupList = ((JXGroupableTableHeader) header).getGroupsForColumn(aColumn);
...
//    }
    ...
    protected void paintGrid(Graphics g, JComponent c) {
        ...
        int lineWeight = (int) Math.ceil(SubstanceSizeUtils.getBorderStrokeWidth(SubstanceSizeUtils.getComponentFontSize(c)));
...
//                BasicStroke.JOIN_BEVEL));
        ...
        if (ltr) {
            for (int column = cMin; column <= cMax; column++) {
                ...
                for (ColumnGroup group : groupList) {
                    ...
                    int bottomGroup = groupRect.y + groupRect.height;

                    if (hasTrailingVerticalGridLine(header, cellRect, column)) {
                        g2d.drawLine(groupRect.x + groupRect.width - 1,
                                groupRect.y, groupRect.x + groupRect.width - 1,
                                bottomGroup);
                    }


                    g2d.drawLine(groupRect.x, (int) (bottomGroup - strokeWidth / 2), groupRect.x + groupRect.width - 1,
                            (int) (bottomGroup - strokeWidth / 2));
                ...
                if (aColumn != draggedColumn) {
                    if (hasLeadingVerticalGridLine(header, cellRect, column)) {
                        g2d.drawLine(cellRect.x, cellRect.y, cellRect.x,
                                bottom);
                    }
                    if (hasTrailingVerticalGridLine(header, cellRect, column)) {
    ...
    protected void paintCell(Graphics g, Rectangle cellRect, int columnIndex) {
        ...
        if (activeStates != null) {
            for (Map.Entry<ComponentState, StateTransitionTracker.StateContributionInfo> stateEntry : activeStates.entrySet()) {
                hasHighlights = (SubstanceColorSchemeUtilities.getHighlightAlpha(this.header, stateEntry.getKey())
        ...
        if (hasHighlights) {
            if (activeStates == null) {
                ...
                if (alpha > 0.0f) {
                    SubstanceColorScheme fillScheme = SubstanceColorSchemeUtilities.getColorScheme(this.header,
                ...
                for (Map.Entry<ComponentState, StateTransitionTracker.StateContributionInfo> stateEntry : activeStates.entrySet()) {
                    ...
                    float alpha = SubstanceColorSchemeUtilities.getHighlightAlpha(this.header, activeState)
    ...
    @Override
    protected void paintCell(Graphics g, Rectangle cellRect, ColumnGroup group) {
        Graphics2D g2d = (Graphics2D) g.create();
        g2d.setComposite(LafWidgetUtilities.getAlphaComposite(header, g));
        ...
        StateTransitionTracker.ModelStateInfo modelStateInfo = getModelStateInfo(group);
        Map<ComponentState, StateTransitionTracker.StateContributionInfo> activeStates = ((modelStateInfo == null) ? null
                : modelStateInfo.getStateContributionMap());
        ComponentState currState = ((modelStateInfo == null) ? getColumnState(group)
                : modelStateInfo.getCurrModelState());
...
//        g2d.dispose();
    ...
    public void update(Graphics g, JComponent c) {
        ...
        SubstanceColorScheme fillScheme = SubstanceColorSchemeUtilities.getColorScheme(c, backgroundState);
        SubstanceColorScheme borderScheme = SubstanceColorSchemeUtilities.getColorScheme(c, ColorSchemeAssociationKind.BORDER,
                backgroundState);
    ...
    public void uninstallUI(JComponent c) {
        for (int i = 0; i < header.getColumnModel().getColumnCount(); i++) {
            ...
            Component rendComp = renderer.getTableCellRendererComponent(header.getTable(), aColumn.getHeaderValue(), false, false, -1, i);
            ...
            if (columnModel.getColumnSelectionAllowed()) {
                ...
                if ((table != null)
                    ...
                    if (hasSelectionAnimations
                            && AnimationConfigurationManager.getInstance().isAnimationAllowed(
    ...
    public ComponentState getColumnState(ColumnGroup group) {
        // ButtonModel synthModel = new DefaultButtonModel();
        boolean toEnable = header.isEnabled();
        ...
        StateTransitionTracker tracker = this.stateTransitionMultiTrackerGroup.getTracker(group);
        if (tracker == null) {
            boolean isRollover = false;
            TableColumnModel columnModel = header.getColumnModel();
            boolean isSelected = false;
            if (columnModel.getColumnSelectionAllowed()) {
                isSelected = columnModel.getSelectionModel().isSelectedIndex(
                        columnModel.getColumnIndex(group.getHeaderValue()));
                if ((table != null)
                        && (table.getUI() instanceof SubstanceTableUI)) {
                    SubstanceTableUI tableUI = (SubstanceTableUI) table.getUI();
                    int rolledOverIndex = tableUI.getRolloverColumnIndex();
                    isRollover = (rolledOverIndex >= 0)
                            && (rolledOverIndex == columnModel.getColumnIndex(group.getHeaderValue()));
                    boolean hasSelectionAnimations = tableUI.hasSelectionAnimations();
                    if (hasSelectionAnimations
                            && AnimationConfigurationManager.getInstance().isAnimationAllowed(
                            AnimationFacet.SELECTION, table)) {
                        isSelected = this.selectedIndices.containsKey(columnModel.getColumnIndex(group.getHeaderValue()));
                    }
                }
            }
            return ComponentState.getState(toEnable, isRollover, isSelected);
    ...
    public StateTransitionTracker getStateTransitionTrackerGroup(ColumnGroup group) {
        return this.stateTransitionMultiTrackerGroup.getTracker(group);
    }
    ...
    protected static class ScrollPaneCornerFiller extends JComponent implements
        ...
        protected void paintComponent(Graphics g) {
            ...
            boolean ltr = header.getComponentOrientation().isLeftToRight();
            final ComponentState backgroundState = (header.isEnabled() && header.getTable().isEnabled()) ? ComponentState.ENABLED
                    : ComponentState.DISABLED_UNSELECTED;
            ...
            float strokeWidth = SubstanceSizeUtils.getBorderStrokeWidth(SubstanceSizeUtils.getComponentFontSize(header));
            g2d.setStroke(new BasicStroke(strokeWidth, BasicStroke.CAP_ROUND,
                    BasicStroke.JOIN_BEVEL));
            g2d.setRenderingHint(RenderingHints.KEY_ANTIALIASING,
                    RenderingHints.VALUE_ANTIALIAS_ON);
    ...
    protected class ColumnHeaderRepaintCallback extends UIThreadTimelineCallbackAdapter {
        ...
        public ColumnHeaderRepaintCallback(JTableHeader header, ColumnGroup group) {
            this.header = header;
            this.group = group;
        }
    ...
    public StateTransitionTracker getTracker(final ColumnGroup group,
            boolean initialRollover, boolean initialSelected) {
        StateTransitionTracker tracker = stateTransitionMultiTrackerGroup.getTracker(group);
        if (tracker == null) {
            ButtonModel model = new DefaultButtonModel();
            model.setSelected(initialSelected);
            model.setRollover(initialRollover);
            ...
            tracker.setRepaintCallback(new RepaintCallback() {
                ...
                @Override
                public TimelineCallback getRepaintCallback() {
                    return new ColumnHeaderRepaintCallback(header, group);
                }
            });
            stateTransitionMultiTrackerGroup.addTracker(group, tracker);
        }
        return tracker;
    }
}
